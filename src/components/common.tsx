  ...
  return (
    <section class="module-panel">
      ...
      <div class="h-12 flex items-center justify-start">
        ...
        <button class="btn btn-sm p-0 w-9 h-9" data-ExtensionPanel-button-title="展开采集详情面板" aria-label="展开采集详情面板" onClick={onClick}>
          <IconArrowUpRight />
        </button>
      </div>
      {/* Modal entries. */}
...
export function Modal({
  ...
  return (
    ...
    >
      <div class="fixed inset-0 bg-black bg-opacity-50" onClick={onClose} />
      <div
        class={`absolute inset-0 bg-white overflow-hidden ${className}`}
      >
        <div class="flex flex-col h-full">
          <div class="flex items-center justify-between p-4 border-b dark:border-gray-700">
            <h2 id="modal-title" class="text-xl text-error font-semibold">{title}</h2>
            <button
